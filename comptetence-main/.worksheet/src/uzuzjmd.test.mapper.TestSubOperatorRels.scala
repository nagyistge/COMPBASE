...
object TestSubOperatorRels {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(1627); 
  ...
  val testrcdeo =  rcdeos.head;System.out.println("""testrcdeo  : uzuzjmd.rcd.generated.Rdceo = """ + $show(testrcdeo ));$skip(128); val res$0 = 
  testrcdeo.getDefinition().asScala.head.getStatement().add(CSV2RCD.createStatement("tippen", CompOntClass.SubOperator.name()));System.out.println("""res0: Boolean = """ + $show(res$0));$skip(40); 
 ...
 ontModel.listClasses().toSet().asScala.map(x=>x.getLocalName());System.out.println("""res1: scala.collection.mutable.Set[String] = """ + $show(res$1))}
