...
object Ont2SuggestedCompetenceGraph {
  ...
  def getHashMap(comp : CompOntologyManager, learningProjectTemplate: LearningProjectTemplate, graph: Graph): HashMap[GraphTriple, java.util.List[String]] = {
    ...
    val result2 = graph.triples.asScala.map { x =>  (x, resultInverted.get(x.fromNode).toSet.intersect(resultInverted.get(x.toNode).toSet).flatten.toList.asJava) }.filterNot(p => p._2.isEmpty).toMap
    
    val result = new HashMap[GraphTriple, java.util.List[String]]
