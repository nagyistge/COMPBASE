...
object EposXMLToSuggestedLearningPath {
  ...
  def convertLevelsToOWLRelations2(descriptorSetType: List[DESCRIPTORType], learningProject: LearningProjectTemplate) {
    ...
    result.foreach(x => convertLevelToOWLRelations3((x.head._1, x.head._2), (x.tail.head._1, x.tail.head._2), learningProject))
  }

  def convertLevelToOWLRelations3(domain: (String, String), range: (String, String), learningProject: LearningProjectTemplate) {
