				...
				Ext.Container,
				{
					...
					addTool : function() {
						...
						for (var E = 0, C = arguments, B = C.length; E < B; E++) {
							...
							if (A.extqtip) {
								if (typeof A.extqtip == "object") {
									Ext.QuickTips.register(Ext.apply({
										target : D.id
									}, A.extqtip))
								} else {
									D.dom.extqtip = A.extqtip
								}
							}
							D.addClassOnOver(G)
						}
		...
		.extend(
				Ext.Panel,
				{
					...
					initTab : function(D, B) {
						...
						if (D.tabTip) {
							Ext.fly(C).child("span.x-tab-strip-text", true).extqtip = D.tabTip
						}
...
Ext.QuickTip = Ext.extend(Ext.ToolTip, {
	...
	onTargetOver : function(G) {
		...
		if (this.interceptTitles && C.title) {
			...
			E = C.extqtip || F.getAttributeNS(D, B.attribute)
		}
		if (E) {
		...
		if (!this.rendered) {
			...
			if (A.extqtip) {
				if (this.textNode.setAttributeNS) {
					this.textNode.setAttributeNS("ext", "extqtip", A.extqtip);
					if (A.extqtipTitle) {
						this.textNode.setAttributeNS("ext", "qtitle",
								A.extqtipTitle)
					}
				} else {
					this.textNode.setAttribute("ext:extqtip", A.extqtip);
					if (A.extqtipTitle) {
						this.textNode.setAttribute("ext:qtitle", A.extqtipTitle)
					}
				}
			} else {
				if (A.extqtipCfg) {
					A.extqtipCfg.target = Ext.id(this.textNode);
					Ext.QuickTips.register(A.extqtipCfg)
				}
			}
			this.initEvents();
