...
import uzuzjmd.owl.competence.ontology.CompOntReasoner;
...
public class OntologyManagerTest {
	...
	public void createIndividuals() throws IOException {
		CompOntologyManager manager = new CompOntologyManager();		
		manager.createBaseOntology();		
		manager.getUtil().createIndividualForString(manager.getUtil().getClass(CompOntClass.Competence), "meineTestkompetenz");
		manager.getUtil().createIndividualForString(manager.getUtil().getClass(CompOntClass.Competence), "meineTestkompetenz2");						
		...
		manager.getUtil().writeOntologyout(manager.getM());
		System.out.println(manager.getM().getResource(MagicStrings.PREFIX+CompOntClass.Competence.name()).as(OntClass.class).listInstances().next().getSameAs());
