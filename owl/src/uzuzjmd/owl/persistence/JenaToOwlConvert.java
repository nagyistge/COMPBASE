...
public class JenaToOwlConvert {
	 ...
	 */
	public synchronized OWLOntology ModelJenaToOwlConvert(Model jenamodel,
			String format) {
		...
		try {
			...
			if (!format.equals("RDF/XML")) {
				System.err
						.println("The only format supported is RDF/XML. Please check the format!");
				...
				OWLOntology owlmodel = owlmanager
	 ...
	 * @return {An OWLOntology that is an owl object}
	 */
	public synchronized OWLOntology ModelJenaToOwlConvert(IRI ontologyIRI,
			Model jenamodel, String format) {
		...
		try {
			...
			if (!format.equals("RDF/XML")) {
				...
				OWLOntology ontology = merger.createMergedOntology(owlmanager,
						ontologyIRI);
	 ...
	 */
	public synchronized OntModel ModelOwlToJenaConvert(OWLOntology owlmodel,
			String format) {
		...
		try {
			...
			if (format.equals("TURTLE") || format.equals("RDF/XML")) {
				...
				if (format.equals("TURTLE"))
					owlmanager.setOntologyFormat(owlmodel,
							new TurtleOntologyFormat());
				if (format.equals("RDF/XML"))
					owlmanager.setOntologyFormat(owlmodel,
							new RDFXMLOntologyFormat());

				OWLOntologyFormat owlformat = owlmanager
						.getOntologyFormat(owlmodel);
				...
				jenamodel.read(new ByteArrayInputStream(out.toByteArray()), id
						.toString().replace("<", "").replace(">", ""), format);
	...
	/**
	 * This function converts every statments relative to a resource in an a set
	 * of OWLAxiom objects
	 * 
	 * @param jenadata
	 *            {A resource in the form (S,P,O), it could be any kind of
	 *            resource (a class, a data property, an object property and an
	 *            instance) except a litteral}
	 * @param format
	 *            {The format of the ontology, i.e. "RDF/XML"}
	 * @return {A set of axiom in the form of Set<OWLAxiom>}
	 */
	public synchronized Set<OWLAxiom> ResourceJenaToOwlAxiom(Resource jenadata,
			String format) {
	...
	/**
	 * This function converts a set of OWLAxiom in an iterator over jena
	 * statements
	 * 
	 * @param axioms
	 *            {A set of aximos}
	 ...
	 */
	public synchronized StmtIterator AxiomOwlToJenaResource(
			Set<OWLAxiom> axioms, String format) {
	...
	/**
	 * This function converts any thingths relatives to an OWL entity in an
	 * iterator over Jena statement
	 * 
	 * @param entity
	 *            {It could be a class, an object property or a data property}
	 * @param owlmodel
	 *            {OWLOntology model where to retrieve information about the
	 *            entity}
	 * @param format
	 *            {RDF/XML or TURTLE}
	...
	public synchronized StmtIterator EntityOwlToJenaResource(OWLEntity entity,
			OWLOntology owlmodel, String format) {
		...
		try {
			...
			if (entity.isOWLClass()) {
				...
				Iterator<OWLClassAxiom> entityaxiom = owlmodel.getAxioms(
						owldata).iterator();

				while (entityaxiom.hasNext())
					manager.addAxiom(ontology, entityaxiom.next());

				Iterator<OWLAnnotationAssertionAxiom> annotations = entity
						.getAnnotationAssertionAxioms(owlmodel).iterator();
			...
			if (entity.isOWLDataProperty()) {
				...
				Iterator<OWLDataPropertyAxiom> entityaxiom = owlmodel
						.getAxioms(owldata).iterator();
			...
			if (entity.isOWLObjectProperty()) {
				...
				Iterator<OWLObjectPropertyAxiom> entityaxiom = owlmodel
						.getAxioms(owldata).iterator();
			...
			if (entity.isOWLDatatype()) {
				...
				Iterator<OWLDatatypeDefinitionAxiom> entityaxiom = owlmodel
						.getAxioms(owldata).iterator();
			...
			if (entity.isOWLNamedIndividual()) {
				...
				Iterator<OWLIndividualAxiom> entityaxiom = owlmodel.getAxioms(
						owldata).iterator();
			...
			if (entity.isOWLAnnotationProperty()) {
				OWLAnnotationProperty owldata = entity
						.asOWLAnnotationProperty();

				Iterator<OWLAnnotationAxiom> entityaxiom = owlmodel.getAxioms(
						owldata).iterator();
	...
	/**
	 * This function converts a single ObjectProperty of Jena to an
	 * OWLObjectProperty of OWLAPI
	 * 
	 * @param jenadata
	 ...
	 */
	public synchronized OWLObjectProperty ObjPropJenaToOwl(
			ObjectProperty jenadata, String format) {
	...
	/**
	 * This function converts a single OWLObjectProperty of owl to an
	 * ObjectProperty of Jena
	 * 
	 * @param data
	...
	public synchronized ObjectProperty ObjPropOwlToJena(OWLObjectProperty data,
			String format) {
	...
	/**
	 * This function converts a DatatypeProperty of Jena to and OWLDataProperty
	 * of owl
	 * 
	 * @param jenadata
	 ...
	 */
	public synchronized OWLDataProperty DataPropJenaToOwl(
			DatatypeProperty jenadata, String format) {
	...
	/**
	 * This function converts a single OWLDataProperty of OWL to
	 * DatatypeProperty of Jena
	 * 
	 * @param data
	 ...
	 */
	public synchronized DatatypeProperty DataPropOwlToJena(
			OWLDataProperty data, String format) {
	...
	/**
	 * This function converts a single AnnotationProperty of Jena to an
	 * OWLAnnotationProperty of OWL
	 * 
	 * @param jenadata
	 *            {Jena DatatypeProperty object}
	 * @param format
	...
	public synchronized OWLAnnotationProperty AnnotationPropJenaToOwl(
			AnnotationProperty jenadata, String format) {
		...
		try {
			...
			available = true;
			notifyAll();
			return owlmodel.getAnnotationPropertiesInSignature().iterator()
					.next();
		} catch (Exception e) {
	...
	/**
	 * This function converts a single OWLAnnotationProperty of OWL to an
	 * AnnotationProperty of Jena
	 * 
	 * @param data
	 ...
	 */
	public synchronized AnnotationProperty AnnotationPropOwlToJena(
			OWLAnnotationProperty data, String format) {
