...
public class DoGetTagsForLit extends DoNeo implements Do {
	...
	public DoGetTagsForLit(GraphDatabaseService graphDB, Index<Node> nodeIndex,
			RelationshipIndex relIndex, Literature literature, Neo4JStarter neo4jStarter) {
	...
	public Graph doit() {
		...
		for (LiteratureEntry literatureEntry : literature.literatureEntries) {
			...
			Iterable<Relationship> rels = litNode.getRelationships(RelTypes.isTagOf);
			for (Relationship relationship : rels) {
				Node graphNode = relationship.getOtherNode(litNode);
				Graph shortestPathGraph = this.neo4JStarter.doQuery(new DoFindShortestPath(this.graphDb, nodeIndex, this.relIndex, "rootnode" , (String) graphNode.getProperty(NODE_VALUE)));
				graph.mergeWith(shortestPathGraph);
			}
		}
		...
		for (GraphNode graphNode : graph.nodes) {
			for (GraphNode graphNode2 : graph.nodes) {
				...
				graph.mergeWith(this.neo4JStarter.doQuery(new DoFindShortestPath(this.graphDb, nodeIndex, this.relIndex, graphNode2.label, graphNode.label)));
			}
		}
		return graph;
	}
